...
import { ENINMAN_edit, ENINMAN_edit_resp } from "@/components/Plantillas";
export function ENINMAN_resp({
  id_pregunta,
  buscar,
  id_nivel,
  AbrirPreguntas,
  ...
  const [respuesta, setRespuesta] = useState("NaN");
  const cerrar1 = (valor) => {
    setError(valor);
  };
  ...
  return (
    <Card className="w-auto mx-auto rounded-none">
      {editarPregunta && (
        <ENINMAN_edit idpregunta={idPreguntaEdit} cerrar={CerrarEdit} />
      )}
      {editRespuesta && (
        <ENINMAN_edit_resp
      ...
      <Dialog open={openNew} handler={hanldeOpen}>
        <DialogBody className="w-full">
          ...
          <Typography
            ...
            color="blue-gray"
          >
            Agregar un espacio para escribir la respuesta el cual esta
            representado por NaN
          </Typography>
