...
export  function MULIMGT({
  ...
  const [ColumnasImg, setColumnasIMG] = useState(0);
  ...
  const Crear_pregunta = async () => {
    ...
    try {
      ...
      form.set("p_columnas", ColumnasImg);

      const result = await axios.post(
        process.env.NEXT_PUBLIC_ACCESLINK + "preguntas/Crear_pregunta_Columnas",
        form,
        {
  ...
  return (
    <Card className="w-auto  mx-auto rounded-none">
      ...
      <CardBody className="flex flex-col gap-4">
        ...
        {/* ANADIR UN CAMPO PARA LAS COLUMNAS QUE SE A PODER OBSERVAR SEGUN LA IMAGEN QUE SE SUBA */}
        <div className="flex items-center">
          <Typography className="text-lg font-bold" color="black">
            Numero de columnas (igual que la imagen):
          </Typography>
          <input
            className="ml-6 w-16 p-2 border  rounded-sm border-yellow-900 "
            type="number"
            onChange={(e) => setColumnasIMG(e.target.value)}
          />
        </div>
      </CardBody>
